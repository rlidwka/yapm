...
var npm = require("./npm.js")
  ...
  , repo = require('github-url-from-git')
  , chalk = require('chalk')
...
function output (data, args) {
  ...
  console.log();
  lines.forEach(function(pkg){
    args.forEach(function (arg) {
      pkg.name = pkg.name.replace(arg, function (arg) {
        return chalk.underline(arg);
      });
      pkg.description = pkg.description.replace(arg, function (arg) {
        return chalk.underline(arg);
      });
    });
    console.log('  \033[36m%s\033[0m', pkg.name);
    if (pkg.repository) console.log('  repo: \033[90m%s\033[0m', repo(pkg.repository.url));
    console.log('  desc: \033[90m%s\033[0m', description(pkg.description));
    console.log('  by: \033[90m%s\033[0m', pkg.author.replace(/=/g, ''));
    console.log();
  });
}
