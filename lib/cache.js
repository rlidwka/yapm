...
function _add(args, cb) {
  ...
  var key = spec.split('@')[0];
  var val = spec.split('@')[1];

  if (~key.indexOf('/')) {
    var token = npm.config.get('github_token');
    ...
    lookup({
      ...
      if (!release) return cb(new Error('failed to find release satisfying ' + val));
      var url = release.tarball_url + '?access_token=' + token;
      add([name, url], cb);
    });
