...
// patch JSON.parse() to use custom fallback
require('jju').utils.patch_JSON_parse()
...
try {
  ...
  var j = jju.parse(fs.readFileSync(
    path.join(__dirname, "../package.json5"))+"")
  npm.name = j.name
  npm.version = j.version
  npm.origVersion = j.npm_version
} catch (ex) {
  try {
...
function load (npm, cli, cb) {
  which(process.argv[0], function (er, node) {
    ...
    npmconf.load(cli, builtin, function (er, config) {
      ...
      ua = ua.replace(/\{npm-version\}/gi, npm.origVersion)
      ua = ua.replace(/\{yapm-version\}/gi, npm.version)
      ...
      switch (npm.config.get('progress')) {
        case "always": progress_enabled = true; break
        case false: progress_enabled = false; break
        default: progress_enabled = npm.color;
      }
      require('yapm-progress').enabled = progress_enabled

      require('package-yaml').setup(npm.config.get('yapm-formats').split(','))
