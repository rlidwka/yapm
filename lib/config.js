...
function del (key, cb) {
  ...
  npm.config.del(key, (where === 'user' && npm.config.get('registry', 'cli')) ? 'user-reg' : where)
...
function set (key, val, cb) {
  ...
  var where = npm.config.get("global") ? "global" : "user"
  npm.config.set(key, val, (where === 'user' && npm.config.get('registry', 'cli')) ? 'user-reg' : where)
  npm.config.save(where, cb)
}

function get (key, cb) {
  if (!key) return list(cb)
  if (key.charAt(0) === "_" || key === 'registries') {
...
function public (k) {
  return !(k.charAt(0) === "_" || types[k] !== types[k] || k === 'registries')
}
...
function list (cb) {
  ...
  // registry overrides
  var uconf = npm.config.sources['user-reg'].data
    , uconfKeys = getKeys(uconf)
  if (uconfKeys.length) {
    msg += "; userconfig.registry " + npm.config.get('registry') + "\n"
