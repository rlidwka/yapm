...
function fetch_ (remote, local, headers, cb) {
  ...
  yapm_progress.wrap(url.parse(remote), req)

  req.on("response", function (res) {
    log.http(res.statusCode, stripToken(remote))
    response = res
    response.resume()
...
// XXX rlidwka: _headers unused?
function makeRequest (remote, fstr, _headers) {
  var hasToken = ~remote.indexOf('?access_token')
  ...
  if (hasToken) {
    var tok = qs.parse(remote.query).access_token;
    headers.Authorization = 'Bearer ' + tok;
    remote.href = stripToken(remote.href);
  }
...
function stripToken(href) {
  return href.replace(/\?access_token=.*/, '');
}
