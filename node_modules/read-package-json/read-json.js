...
function readJson (file, log_, strict_, cb_) {
                ...
                if (c) {
                                cb = cb.bind(null, null, c.data, c.source)
                                return process.nextTick(cb);
                }
                cb = (function (orig) { return function (er, data, source) {
                                if (data) readJson.cache.set(file, {data: data, source: source});
                                return orig(er, data, source)
                } })(cb)
...
function parseJson (file, er, raw, log, strict, cb) {
                ...
                if (er) return cb(er);
                try {
                                var d = JSON.parse(stripBOM(raw))
                } catch (er) {
                                var d = parseIndex(raw)
                                if (!d) return cb(parseError(er, file));
                }
                extras(file, d, {raw: raw, file: file}, log, strict, cb)
}
...
function indexjs (file, er, log, strict, cb) {
                ...
                fs.readFile(index, "utf8", function (er2, raw) {
                                if (er2) return cb(er);
                                var d = parseIndex(raw)
                                if (!d) return cb(er);
                                extras(file, d, {raw: raw, file: index}, log, strict, cb)
                })
}


readJson.extras = extras
function extras (file, data, source_, log_, strict_, cb_) {
                var source, log, strict, cb
                ...
                function then(er) {
                                ...
                                if (--n > 0) return;
                                final(file, data, source, log, strict, cb);
                }
}
...
function final (file, data, source, log, strict, cb) {
                ...
                typoWarned[pId] = true
                readJson.cache.set(file, {data: data, source: source})
                cb(null, data, source)
}
