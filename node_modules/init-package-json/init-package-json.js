  ...
  readJson(package, function (er, d) {
    ...
    pz.on('data', function (data) {
      ...
      readJson.extras(package, pkg, function (er, pkg) {
        ...
        function write (yes) {
          pkgyaml.write(dir, pkg, function (er) {
            if (!er && yes) console.log('Wrote to %s:\n\n%s\n', dir, util.inspect(pkg))
            return cb(er, pkg)
          })
        }
        if (ctx.yes) {
        ...
        function aboutToWrite(where) {
          console.log('About to write to %s:\n\n%s\n', dir + '/' + where, util.inspect(pkg))
        ...
        pkgyaml.stat(dir, function(err, data) {
          if (err || !data) {
            read({
              prompt: 'format: ',
              default: 'json'
            }, function (er, format) {
              pkgyaml.setup([ format, 'json' ])
              aboutToWrite('package.' + format)
            })
          } else {
            aboutToWrite(path.basename(data.file))
          }
        })
      })
    })
