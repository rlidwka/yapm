...
Conf.prototype.save = function (where, cb) {
  ...
  mask_passwords(data)
  if (typeof(data.registries) === 'object' && data.registries != null) {
    for (var i in data.registries) {
      data.registries[i] = mask_passwords(data.registries[i])
    }
  }
...
Conf.prototype.add = function (data, marker) {
  ...
  unmask_password(data)

  CC.prototype.add.call(this, data, marker)

  return this
}

Conf.prototype.reset_registries = function () {
  var val = this.get('registry')
  registry = url.parse(String(val))
  if (!registry.host) throw new Error('Invalid registry: ' + val)
  registry = registry.href

  for (var marker in this.sources) {
    var olddata = this.sources[marker].data
    var marker2
    if (Object.prototype.hasOwnProperty.call(olddata, 'registries')
    &&  Object.prototype.hasOwnProperty.call(olddata.registries, registry)) {
      data = olddata.registries[registry]
      unmask_password(data)
    } else {
      if (marker !== 'user') continue
      if (typeof(olddata.registries) !== 'object' || olddata.registries == null) {
        olddata.registries = {}
      }
      data = olddata.registries[registry] = {}
    }
    ...
    if (typeof(marker) === 'object') {
      marker2 = marker.__source__ + '-reg'
    } else {
      marker2 = marker + '-reg'
    }

    if (this.sources[marker2]) {
      var i = this.list.indexOf(olddata)
      if (i === -1) {
        return this.emit('error', new Error('data not inserted?'))
      }
      this.splice(i-1, 1)
    }

    this.sources[marker2] = this.sources[marker2] || {}
    this.sources[marker2].data = data
